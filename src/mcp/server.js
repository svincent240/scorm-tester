...
const TOOL_META = new Map([
  ...
  ["scorm_automation_get_audio_state", { description: "Get comprehensive audio state: hasAudio (boolean), audioState (source, position, duration, isPlaying, contextType, isCompleted), and progress (0-100). Returns null values if no audio loaded.", inputSchema: { type: "object", properties: { session_id: { type: "string" } }, required: ["session_id"] } }],
  ["scorm_automation_is_audio_completed_for_context", { description: "Check if audio for a specific context (slideId, modal-xxx, etc.) has reached completion", inputSchema: { type: "object", properties: { session_id: { type: "string" }, context_id: { type: "string" } }, required: ["session_id", "context_id"] } }],
  ["scorm_automation_simulate_audio_complete", { description: "Simulate audio completion for testing - seeks to threshold AND triggers engagement tracking. Handles all context types: slide→slideAudioComplete, modal→modalAudioComplete, standalone→audioComplete", inputSchema: { type: "object", properties: { session_id: { type: "string" } }, required: ["session_id"] } }],

...
const TOOL_META = new Map([
  ...
  ["scorm_automation_get_audio_state", { description: "Get comprehensive audio state including availability (hasAudio), playback status (isPlaying, position, duration), completion status (isCompleted, isRequired), and mute state using Template Automation API", inputSchema: { type: "object", properties: { session_id: { type: "string" } }, required: ["session_id"] } }],
  ["scorm_automation_audio_control", { description: "Control audio playback with actions: play, pause, toggle, restart, seek (requires seconds param), seek_percentage (requires percentage param 0-100), mute, unmute, toggle_mute", inputSchema: { type: "object", properties: { session_id: { type: "string" }, action: { type: "string", enum: ["play", "pause", "toggle", "restart", "seek", "seek_percentage", "mute", "unmute", "toggle_mute"] }, seconds: { type: "number", minimum: 0 }, percentage: { type: "number", minimum: 0, maximum: 100 } }, required: ["session_id", "action"] } }],
  ["scorm_automation_is_audio_completed_for_context", { description: "Check if audio for a specific context (e.g., slide-01) has reached completion using Template Automation API", inputSchema: { type: "object", properties: { session_id: { type: "string" }, context_id: { type: "string" } }, required: ["session_id", "context_id"] } }],
  ["scorm_automation_simulate_audio_complete", { description: "Simulate audio completion for testing (seeks to completion threshold) using Template Automation API - critical for fast test execution without waiting for playback", inputSchema: { type: "object", properties: { session_id: { type: "string" } }, required: ["session_id"] } }],
  ...
  router.register("scorm_automation_get_audio_state", scorm_automation_get_audio_state);
  router.register("scorm_automation_audio_control", scorm_automation_audio_control);
  router.register("scorm_automation_is_audio_completed_for_context", scorm_automation_is_audio_completed_for_context);
  router.register("scorm_automation_simulate_audio_complete", scorm_automation_simulate_audio_complete);

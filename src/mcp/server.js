...
const TOOL_META = new Map([
  ...
  ["scorm_get_console_errors", { description: "Get browser console errors/warnings. Returns error count by default; set include_errors=true to get full error details (to reduce token bloat).", inputSchema: { type: "object", properties: { session_id: { type: "string" }, since_ts: { type: "number" }, severity: { type: "array", items: { type: "string", enum: ["error", "warn", "info"] } }, include_errors: { type: "boolean" }, limit: { type: "number" } }, required: ["session_id"] } }],
